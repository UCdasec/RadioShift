function modulator = dlhdlhelperModClassGetModulator(modType, sps, fs)
...
switch modType
  ...
  case "16APSK"                               %Extrenally 16APSK Introduced
    modulator = @(x)apsk16Modulator(x,sps);
  case "32APSK"                               %Extrenally 32APSK  Introduced
    modulator = @(x)apsk32Modulator(x,sps);
  case "64APSK"                               %Extrenally 64APSK  Introduced
    modulator = @(x)apsk64Modulator(x,sps);
  case "128APSK"                              %Extrenally 128APSK Introduced
    modulator = @(x)apsk128Modulator(x,sps);
  case "16QAM"
    modulator = @(x)qam16Modulator(x,sps);
    case "32QAM"                            %Extrenally 32QAM Introduced
    modulator = @(x)qam32Modulator(x,sps);
  case "64QAM"
    modulator = @(x)qam64Modulator(x,sps);
  case "128QAM"                             %Extrenally 128 QAM Introduced
    modulator = @(x)qam128Modulator(x,sps);
  case "256QAM"                             %Extrenally 256 QAM Introduced
...
%psk8Modulator 16-APSK modulator with pulse shaping
%   Y = apsk16Modulator(X,SPS) 16-APSK modulates the input X, and returns the
%   root-raised cosine pulse shaped signal Y. X must be a column vector
...
%   apsk32Modulator 32-APSK modulator with pulse shaping
%   Y = apsk16Modulator(X,SPS) 32-APSK modulates the input X, and returns the
%   root-raised cosine pulse shaped signal Y. X must be a column vector
%   of values in the set [0 31]. The root-raised cosine filter has a
%   roll-off factor of 0.35 and spans four symbols. The output signal
%   Y has unit power.
...
%   apsk64Modulator 64-APSK modulator with pulse shaping
%   Y = apsk16Modulator(X,SPS) 64-APSK modulates the input X, and returns the
%   root-raised cosine pulse shaped signal Y. X must be a column vector
...
%   apsk128Modulator 128-APSK modulator with pulse shaping
%   Y = apsk128Modulator(X,SPS) 128-APSK modulates the input X, and returns the
%   root-raised cosine pulse shaped signal Y. X must be a column vector
%   of values in the set [0 127]. The root-raised cosine filter has a
%   roll-off factor of 0.35 and spans four symbols. The output signal
%   Y has unit power.
...
function y = qam32Modulator(x,sps) %Introduced Externally
%qam16Modulator 32-QAM modulator with pulse shaping
%   Y = qam32Modulator(X,SPS) 32-QAM modulates the input X, and returns the
%   root-raised cosine pulse shaped signal Y. X must be a column vector
%   of values in the set [0 31]. The root-raised cosine filter has a
%   roll-off factor of 0.35 and spans four symbols. The output signal
%   Y has unit power.
...
function y = qam128Modulator(x,sps) %Introduced Externally
%qam64Modulator 128-QAM modulator with pulse shaping
%   Y = qam64Modulator(X,SPS) 128-QAM modulates the input X, and returns the
%   root-raised cosine pulse shaped signal Y. X must be a column vector
%   of values in the set [0 127]. The root-raised cosine filter has a
%   roll-off factor of 0.35 and spans four symbols. The output signal
%   Y has unit power.
...
function y = qam256Modulator(x,sps) %Introduced Externally
%qam64Modulator 256-QAM modulator with pulse shaping
%   Y = qam64Modulator(X,SPS) 256-QAM modulates the input X, and returns the
%   root-raised cosine pulse shaped signal Y. X must be a column vector
%   of values in the set [0 255]. The root-raised cosine filter has a
%   roll-off factor of 0.35 and spans four symbols. The output signal
%   Y has unit power.
...
persistent mod prev_input_size
input_size = length(x);
if (isempty(mod) || mod.SampleRate ~= fs || ~isequal(input_size, prev_input_size))
    if(~isempty(mod))
        release(mod);
    end
...
%dsbamModulator Double sideband AM modulat or
